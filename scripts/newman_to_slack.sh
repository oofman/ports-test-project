...
environment='test/newman_collection/Shypple.postman_environment.json'
collection='test/newman_collection/Shypple.postman_collection.json'
global=''
webhook='https://hooks.slack.com/services/{SOME_TOKEN}'
additional=''
...
newman_args=' --reporter-cli-no-failures --reporter-cli-no-assertions --reporter-cli-no-success-assertions --reporter-cli-no-banner'
...
main () {
    ...
    # post to slack
    curl -X POST --data-urlencode 'payload={"icon_emoji": ":test:", "channel":"#channel", "username":"Regression Testing Bot","text": "'"$date"':```'"$output"'```"}' $webhook
}
